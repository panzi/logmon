...
class ActionConfigBase(TypedDict):
    ...
    http_max_redirect: Annotated[NotRequired[int], Field(default=DEFAULT_HTTP_MAX_REDIRECT, ge=0)]
    http_timeout: Annotated[NotRequired[Optional[float]], Field(description="`None` means no timeout.", default=None, ge=0.0)]
    ...
    command_timeout: Annotated[NotRequired[Optional[float]], Field(description="Timeout in seconds. If the timeout expires the process is killed.", default=None, ge=0.0)]
...
class LimitsConfig(TypedDict):
    max_emails_per_minute: Annotated[NotRequired[int], Field(default=DEFAULT_MAX_EMAILS_PER_MINUTE, gt=0)]
    max_emails_per_hour: Annotated[NotRequired[int], Field(default=DEFAULT_MAX_EMAILS_PER_HOUR, gt=0)]
...
class LogfileConfig(TypedDict):
    ...
    wait_line_incomplete: Annotated[NotRequired[int | float], Field(default=DEFAULT_WAIT_LINE_INCOMPLETE, ge=0)]
    wait_file_not_found: Annotated[NotRequired[int | float], Field(default=DEFAULT_WAIT_FILE_NOT_FOUND, ge=0)]
    wait_no_entries: Annotated[NotRequired[int | float], Field(default=DEFAULT_WAIT_NO_ENTRIES, ge=0)]
    wait_before_send: Annotated[NotRequired[int | float], Field(default=DEFAULT_WAIT_BEFORE_SEND, ge=0)]
    wait_after_crash: Annotated[NotRequired[int | float], Field(default=DEFAULT_WAIT_AFTER_CRASH, ge=0)]
    max_entries: Annotated[NotRequired[int], Field(default=DEFAULT_MAX_ENTRIES, ge=0)]
    max_entry_lines: Annotated[NotRequired[int], Field(default=DEFAULT_MAX_ENTRY_LINES, ge=0)]
    ...
    output_indent: Annotated[NotRequired[Optional[int]], Field(description="Indent JSON/YAML log entries in output. If `None` the JSON documents will be in a single line.", default=DEFAULT_OUTPUT_INDENT, ge=0)]
